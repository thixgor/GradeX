...
import type { EcgAnnotColors } from '@/lib/ecg/paper'
...
interface MeasureSpec {
  ...
  normal: string
  /** chave de cor na paleta do papel — resolvida conforme claro/verde */
  color: keyof EcgAnnotColors
  what: string
  longer: string
  shorter: string
}
...
const MEASURES: MeasureSpec[] = [
  {
    ...
    normal: '< 120 ms e < 2,5 mm', color: 'sky',
  ...
  },
  {
    ...
    normal: 'Isoelétrico', color: 'amber',
  ...
  },
  {
    ...
    normal: '< 120 ms (habitual 60–100 ms)', color: 'emerald',
  ...
  },
  {
    ...
    normal: 'Isoelétrico (± 1 mm)', color: 'orange',
  ...
  },
  {
    ...
    normal: 'QTc ≤ 450 ms (H) / ≤ 460 ms (M)', color: 'rose',
  ...
  },
  {
    ...
    normal: 'Regular, com variação < 10%', color: 'violet',
  ...
  },
  {
    ...
    normal: 'Isoelétrico — a referência zero', color: 'slate',
...
export function IntervalLab() {
  ...
  const tone = palette.annot[spec.color]
  ...
  return (
    <LabShell>
      ...
      >
        ...
        <rect x={xOf(spec.from)} y={0} width={xOf(spec.to) - xOf(spec.from)} height={H} fill={tone} opacity="0.14" />
        <line x1={xOf(spec.from)} y1={0} x2={xOf(spec.from)} y2={H} stroke={tone} strokeWidth="1.6" />
        <line x1={xOf(spec.to)} y1={0} x2={xOf(spec.to)} y2={H} stroke={tone} strokeWidth="1.6" />
        <g>
          <line x1={xOf(spec.from)} y1={26} x2={xOf(spec.to)} y2={26} stroke={tone} strokeWidth="1.8" />
          <text
            x={Math.min(W - 92, Math.max(92, (xOf(spec.from) + xOf(spec.to)) / 2))}
            y={19} textAnchor="middle" fontSize="13" fontWeight="900"
            fill={tone} fontFamily="ui-monospace, monospace"
          >
            {spec.label} · {target} ms
          </text>
        </g>

        <line x1={0} y1={BASE} x2={W} y2={BASE} stroke={palette.baseline} strokeWidth="1" strokeDasharray="4 5" />
          ...
          <g>
            {[{ t: a, k: 'a' as const }, { t: b, k: 'b' as const }].map(({ t, k }) => (
              ...
              >
                ...
                <line x1={xOf(t)} y1={0} x2={xOf(t)} y2={H} stroke={palette.annot.pink} strokeWidth="2.4" />
                <rect x={xOf(t) - 17} y={H - 40} width="34" height="36" rx="8" fill={palette.annot.pink} />
                <text x={xOf(t)} y={H - 16} textAnchor="middle" fontSize="17" fontWeight="900" fill={palette.bg}>{k.toUpperCase()}</text>
              </g>
            ))}
            <line x1={xOf(Math.min(a, b))} y1={48} x2={xOf(Math.max(a, b))} y2={48} stroke={palette.annot.pink} strokeWidth="2" />
            <text x={(xOf(a) + xOf(b)) / 2} y={42} textAnchor="middle" fontSize="13" fontWeight="900" fill={palette.annot.pink} fontFamily="ui-monospace, monospace">
              {measured} ms
            </text>
          </g>
        )}
