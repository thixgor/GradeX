...
/**
 * Resolve a recipients descriptor into a deduplicated list of {email, name}.
 * Re-evaluated at send time so scheduled emails always reflect the current
 * user base (and any edits made to the recipient list).
 */
export async function resolveRecipients(recipients: EmailRecipients): Promise<ResolvedRecipient[]> {
    const db = await getDb()
    const usersCollection = db.collection<User>('users')

    let recipientList: ResolvedRecipient[] = []
    ...
    if (recipients.selectAll) {
        ...
        const objectIds = recipients.userIds
            .map(id => {
...
/**
 * Resolves recipients, builds the marketing template and sends it in batches.
 * Shared by the immediate send route and the scheduled-emails cron.
 */
export async function sendMarketingEmail({
    ...
    content,
    previewText,
}: SendMarketingEmailParams): Promise<SendStats> {
    const recipientList = await resolveRecipients(recipients)

    if (recipientList.length === 0) {
        return { total: 0, sent: 0, failed: 0, errors: ['Nenhum destinatário válido encontrado'] }
    }
