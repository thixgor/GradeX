...
const conversorBenzo: Ferramenta = {
  ...
  campos: [
    ...
    campoNum('dose', 'Dose diária total', { ajuda: 'Dose diária total em miligramas, somando todas as tomadas. Se houver mais de um benzodiazepínico, converta cada um separadamente e some os equivalentes.', unidade: 'mg/dia', min: 0.1, max: 200, passo: 0.25 }),
  ],
  calcular: (v) => {
    const de = opc(v, 'de')
    const dose = num(v, 'dose')
...
const receita: Ferramenta = {
  ...
  campos: [
    ...
    campoSimNao('farmaco', 'Fármaco por nome genérico (denominação comum brasileira)', 1, 'Denominação comum brasileira. Nome comercial na prescrição impede a substituição por genérico e é fonte de erro quando a marca muda de composição.'),
    campoSimNao('concentracao', 'Concentração e forma farmacêutica especificadas', 1, 'Ex.: \'amoxicilina 250 mg/5 mL suspensão\'. A ausência da concentração obriga quem administra a converter mg em mL de cabeça — origem principal dos erros de ordem de grandeza em pediatria.'),
    campoSimNao('quantidade', 'Quantidade total por extenso e em algarismos', 1, 'Por extenso e em algarismos, para impedir adulteração e erro de leitura.'),
    campoSimNao('posologia', 'Posologia com dose, via, intervalo e duração', 1, 'Dose, via, intervalo e duração. \'Se necessário\' sem dose máxima diária é prescrição incompleta.'),
    ...
    campoSimNao('alergias', 'Alergias verificadas', 1, 'Alergia verificada e registrada, inclusive reação cruzada entre betalactâmicos e sensibilidade a excipientes.'),
