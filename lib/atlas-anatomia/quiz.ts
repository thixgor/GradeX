...
/**
 * Quiz de identificação do Atlas de Anatomia.
 *
...
function ocorrenciasDe(sistema: AtlasSystem): Ocorrencia[] {
...
let cacheOcorrencias: Ocorrencia[] | null = null

function todasOcorrencias(): Ocorrencia[] {
  if (!cacheOcorrencias) cacheOcorrencias = ATLAS_SYSTEMS.flatMap(ocorrenciasDe)
...
export function regioesDoSistema(sistemaSlug: string): Array<{ nome: string; estruturas: number }> {
...
export function contarEstruturas(recorte: RecorteQuiz): number {
  return todasOcorrencias().filter(
    ocorrencia =>
...
export function montarQuiz(recorte: RecorteQuiz, quantidade: number, semente: number): QuestaoQuiz[] {
  const sortear = gerador(semente)
  const universo = todasOcorrencias().filter(
    ocorrencia =>
      (!recorte.sistemaSlug || ocorrencia.sistemaSlug === recorte.sistemaSlug) &&
      (!recorte.regiao || ocorrencia.regiao === recorte.regiao),
  )
