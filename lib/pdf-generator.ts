...
export interface PurchaseReceiptItem {
  _id: string
  itemTitle: string
  itemType: 'material' | 'package' | 'manual_clinico' | 'subscription'
  price: number
...
export async function generatePurchaseReceiptPDF(
  ...
  purchases.forEach((p, i) => {
    ...
    const typeLabel = p.itemType === 'subscription' ? 'Assinatura' : p.itemType === 'manual_clinico' ? 'Produto' : p.itemType === 'package' ? 'Pacote' : 'Material'
