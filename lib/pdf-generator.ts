...
export function generateGabaritoPDF(exam: Exam): Blob {
  ...
  // Cores
  const primaryColor = [147, 51, 234] as const // Roxo
  const secondaryColor = [59, 130, 246] as const // Azul
  const grayColor = [156, 163, 175] as const // Cinza
  const lightGray = [243, 244, 246] as const // Cinza claro
  ...
  doc.text('DomineAqui', pageWidth / 2, 20, { align: 'center' })
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.text(
      'Gerado pela plataforma DomineAqui',
      margin,
      pageHeight - 10
    )
    doc.text(
...
export function generateAnnotationsPDF(
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'normal')
  doc.text('ANOTAÇÕES DA PROVA', pageWidth / 2, 32, { align: 'center' })
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFont('helvetica', 'normal')
    doc.text(
      `Página ${i} de ${totalPages} - Anotações da Prova`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    )
