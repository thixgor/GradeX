...
/**
 * `tipos` e não `@/lib/tomografia`.
 *
 * O barril importa, no topo, os vinte arquivos de `conteudo/` — 710 KB de atlas.
 * Importar qualquer coisa dele de dentro de um componente cliente colocava o
 * atlas inteiro no bundle do navegador para exibir uma série. `tipos.ts` tem
 * 5 KB, não importa dado nenhum, e é onde `caminhoDoCorte` mora.
 */
import { caminhoDoCorte, type EstruturaTC, type SubsecaoTC } from '@/lib/tomografia/tipos'
...
/** Referência magra a uma série vizinha: só o que o rodapé de navegação usa. */
export interface VizinhaTC {
  id: string
  titulo: string
}

export interface VistaDaSerieProps {
  sub: SubsecaoTC
  /**
   * Só o título da seção, não a `SecaoTC`.
   *
   * `SecaoTC` carrega `subsecoes: SubsecaoTC[]` — a seção inteira. Passá-la
   * como prop devolveria pela serialização o que a troca do barril acabou de
   * tirar do bundle. O mesmo vale para as vizinhas: `getVizinhas` devolve duas
   * `SubsecaoTC` completas e a navegação usa `id` e `titulo` de cada uma.
   */
  secaoTitulo: string | null
  anterior: VizinhaTC | null
  proxima: VizinhaTC | null
}

export function VistaDaSerie({ sub, secaoTitulo, anterior, proxima }: VistaDaSerieProps) {
      ...
      <div className="surface-page min-h-screen">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-8">
          <VitrineRadiologia
            ...
            resumo={dados?.resumo}
            alvo={secaoTitulo ? `A série ${sub.titulo} (${secaoTitulo})` : `A série ${sub.titulo}`}
          />
        </div>
      </div>
    )
  ...
  return (
    <div className="surface-page min-h-screen pb-20 xl:pb-0">
      ...
      <div className="border-b border-border bg-muted/30">
        <div className="rx-abaixo-flutuantes container mx-auto max-w-7xl px-4 pb-5 pt-6">
          ...
          <nav className="mb-4 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <ChevronRight className="h-3 w-3 opacity-40" />
            <span className="font-medium text-foreground/70">{secaoTitulo}</span>
          </nav>
      ...
      <CadernoRadiologico
        ...
        chave={sub.id}
        titulo={sub.titulo}
        subtitulo={secaoTitulo ? `${secaoTitulo} · ${sub.totalCortes} cortes` : `${sub.totalCortes} cortes`}
...
function Roteiro({ sub }: { sub: SubsecaoTC }) {
  return (
    <div className="space-y-4">
