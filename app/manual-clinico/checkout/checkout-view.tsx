...
export type PlanKey = 'semestral' | 'anual' | 'vitalicio'
...
/**
 * O checkout do Manual Clínico.
 *
 * `planQuery` vem como prop, resolvida no servidor por `page.tsx`, e não de
 * `useSearchParams()`: o hook fazia a rota inteira desistir da renderização no
 * servidor, então quem chegava ao checkout via um spinner de tela cheia antes
 * de qualquer preço aparecer. Numa página de compra, essa espera custa venda.
 */
export default function ManualClinicoCheckoutView({
  planQuery,
}: {
  planQuery: PlanKey | null
}) {
