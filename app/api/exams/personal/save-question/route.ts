...
/**
 * POST - Salva uma questão individual em uma prova pessoal
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validar se a prova pessoal existe e pertence ao usuário
    ...
    if (!exam) {
      return NextResponse.json(
        { error: 'Prova pessoal não encontrada ou sem permissão' },
        { status: 404 }
      )
    }
    ...
    console.error('Erro ao salvar questão pessoal:', error)
    return NextResponse.json(
